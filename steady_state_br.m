...
%x0=[0;0.5]%zeros(ndim,1);
    ...
    'max_bound',   [in.sigma, 8;     in.mu_n, 2.5; in.a,1],...
    'max_step',    [in.sigma, 0.05;   in.mu_n, 0.05; in.a, 0.05; 0,0.05]};
%%
funcs=set_symfuncs(@sym_energy_E,'sys_tau',@()[]);
%%
...
%% plot stability
xpar_br1=arrayfun(@(x)x.parameter(in.sigma),eq_branch1.point);
...
%%
pt=eq_branch1.point(biflocation);
...
%%
pt2=eq_branch2.point(biflocation2(2));
Fode45=@(t,X)funcs.wrap_rhs(X,pt2.parameter);
rng(2)
simulode45=ode45(Fode45,[0,50000],pt2.x+0.01*rand(length(pt2.x),1),odeset('RelTol',1e-7,'AbsTol',1e-7));
%
figure(88)
clf;
hold on
plot(simulode45.y(1,:),simulode45.y(2,:),'LineWidth',2)